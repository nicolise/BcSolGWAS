...
#NO, this orders position as a character vector!
...
#823306 to 828345
#MAKE POS a number, currently it's a character vector ugh
mySNPs_Chr2$POS <- as.numeric(paste(mySNPs_Chr2$POS))
mySNPs_Chr2 <- mySNPs_Chr2[order(mySNPs_Chr2$POS),]
...
mySNPlist_8a <- read.csv("data/Bcgenome/chr2_analysis/SNPlistFig8a.csv")
...
g
write.csv(mySNPlist, "data/Bcgenome/chr2_analysis/SNPlistfromPED_EDIT.csv")
...
names(mydf)[1] <- "Pos"
mydf$SNPnum <- c(1:32)
write.csv(mydf, "data/Bcgenome/chr2_analysis/PLINK/MatchDrawLines_EDIT.csv")
...
#make marker loci binary for PED
#REF is row 90 right now
#by column
for (i in 7:ncol(myPED.2)){
  #first have to add levels to the factor!!
  levels(myPED.2[,i]) <- c(levels(myPED.2[,i]),2,1)
  #by row
  for (j in c(1:89,91:98)){
    myPED.2[j,i] <- ifelse(myPED.2[j,i]==myPED.2[90,i], 1,2)
  }
}
#now remove REF
myPED.2 <- myPED.2[-c(90),]
...
#finally, make the PED file for Haploview
...
setwd("~/Projects/BcSolGWAS")
#MAP
write.table(myMAP, file="data/Bcgenome/chr2_analysis/PLINK/myCHR2_A_EDIT.map", quote=FALSE, sep='\t', col.names=FALSE, row.names=FALSE)
myMAP <- read.table("data/Bcgenome/chr2_analysis/PLINK/myCHR2_A.map")

#PED
write.table(myPED.2, file="data/Bcgenome/chr2_analysis/PLINK/myCHR2_A_EDIT.ped", quote=FALSE, sep=' ', col.names=FALSE, row.names=FALSE)
write.table(myPED.null, file="data/Bcgenome/chr2_analysis/PLINK/myCHR2_A.nullPheno_EDIT.ped", quote=FALSE, sep='\t', col.names=FALSE, row.names=FALSE)
write.table(myPED.bin, file="data/Bcgenome/chr2_analysis/PLINK/myCHR2_A.binPheno_EDIT.ped", quote=FALSE, sep='\t', col.names=FALSE, row.names=FALSE)
