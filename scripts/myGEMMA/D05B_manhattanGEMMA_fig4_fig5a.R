...
data/GEMMA_files/D_04_randphenos/

#LA2093 is pheno 6 (see Phenos_match)
Phenos_match <- read.csv("data/GEMMA_files/D_02_randGEMMA/binMAF20NA10_fam.csv")

myGEMMA <- read.table("data/GEMMA_files/03_GEMMAouts/GEMMA_lmm_ind12plants/binMAF20NA10_ind12plants_PLINK_kmat1_pheno4.assoc.txt", header=TRUE)

#check phenotype order here
setwd("~/Documents/GitRepos/BcSolGWAS/data/")
Phenos <- read.csv("data/GWAS_files/02_csvPrep/phenos/NewModel0711/BcSl_lsmeans_forbigRR.csv")
#"LA1547","LA1589","LA1684","LA2093","LA2176","LA2706","LA3008","LA3475","LA410","LA4345","LA4355","LA480"
myFAM <- read.table("data/GEMMA_files/B_01_PLINK/ind12plants/binMAF20NA10.fam")
...
jpeg(paste("paper/plots/addGEMMA/SlBc_MAF20_10NA_GEMMA_LA2093_kmat1_pscore.jpg", sep=""), width=8, height=5, units='in', res=600)
...
print(ggplot(myGEMMA, aes(x=Index, y=(-log(p_score))))+
        ...
        geom_text(aes(0,-log(0.01), label="p = 0.01", vjust = 1, hjust = -0.1), col= "white")+
...
#------------------------------------------------------------------------------ 
#now plot for 4b
  setwd("~/Projects/BcSolGWAS/data/GEMMA_files/03_GEMMAouts/GEMMA_lmm_ind12plants/kmat1")
  #read in each of 12 phenotypes
  my.files <- list.files(pattern = c("assoc"))
  #2_LA1589 fails with both k-matrix options.
  my.names <- c("1_LA1547","10_LA4345", "11_LA4355", "12_LA0480", "2_LA1589","3_LA1684","4_LA2093","5_LA2176","6_LA2706","7_LA3008","8_LA3475","9_LA0410")
#  "LA1547","LA1589","LA1684","LA2093","LA2176","LA2706","LA3008","LA3475","LA410","LA4345","LA4355","LA480"
  #rename all files
...
write.csv(HOSNP, "12Plants_HiOverlapSNPs_trueMAF20_10NA_GEMMA_kmat1.csv")
...
for (y in c(10,13,16,19,22,25,28,31,34,37,40)){
  #sort data frame with small p values to high for each phenotype
...
write.csv(top1ksnp, "12Plants_top1kSNPs_MAF20_10NA_GEMMA_kmat1.csv")
...
#find top SNPs to mark: pheno.bin$SUMM and LA2093 pscore
  pheno.bin.top <- pheno.bin[pheno.bin$`4_LA2093_pscore` < 0.001, ]
  #top 100 beta out of these
  library(plyr)
  pheno.bin.top <- head(arrange(pheno.bin.top, desc(`4_LA2093_beta`)), n=100)
...
jpeg("S3a_topSNPOverlap_12Plants_GEMMA.jpg", width=8, height=5, units='in', res=600)
...
#small inset plot
jpeg("S3a_topSNPOverlap_12Plants_GEMMA_inset.jpg", width=4, height=3, units='in', res=600)
ggplot(pheno.bin, aes(pheno.bin$SUMM)) + 
  geom_bar()+
  theme_bw()+
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
