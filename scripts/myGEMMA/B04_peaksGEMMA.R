...
myGEMMA.D <- read.table("data/GEMMA_files/03_GEMMAouts/binMAF20NA10_PLINK_kmat2_1.assoc.txt", header=TRUE)
myGEMMA.W <- read.table("data/GEMMA_files/03_GEMMAouts/binMAF20NA10_PLINK_kmat2_2.assoc.txt", header=TRUE)
myGEMMA.S <- read.table("data/GEMMA_files/03_GEMMAouts/binMAF20NA10_PLINK_kmat2_3.assoc.txt", header=TRUE)
...
#now read in peaks from bigRR T4
...
jpeg(paste("paper/plots/addGEMMA/SlBc_MAF20_10NA_GEMMA_Domest.jpg", sep=""), width=8, height=5, units='in', res=600)
  print(ggplot(myGEMMA, aes(x=Index, y=beta))+
          ...
          #0.0312 for Sens (3), 0.0518 for Dom (1)
