...
tiff("plots/poster/DomestRight/Sl_LesionSize_beanplots.tiff", width=7.5, height=5.4, units='in', res=600)
...
#color plot
tiff("plots/poster/Sl_LesionSize_IntMean_DW.tif", width=5, height=4, units='in', res=600)
...
#double check:
#D: LA4345, LA3008, LA4355, LA2706, LA3475, LA0410
#W: LA1547, LA2093, LA1684, LA1589, LA0480, LA2176
...
p1 <- ggplot(FigDat3, aes(x = Plant.Lab.Ord, y = mLS))+ 
  theme_bw()+ 
 geom_line(size=0.5, aes(color=factor(mmLS), group=factor(Igeno)), show.legend=F, alpha=0.4)+
  facet_grid(.~SpLabs, scales="free_x")+ 
  theme(text = element_text(size=14), axis.text.x = element_text(angle = 45, hjust = 1), strip.background = element_blank(), aspect.ratio=1/1, axis.title.x=element_text(color=NA), plot.margin=unit(c(1,1,1,1),"lines"))+ 
...
p2 <- ggplot(FigDat3, aes(x = Plant.Lab.Ord, y = mLS))+
  ...
  facet_grid(.~SpLabs, scales="free_x")+
  theme(text = element_text(size=14), axis.text.x = element_text(angle=45, hjust=1), axis.title.y = element_text(), strip.background = element_blank(), axis.title.x=element_text(color=NA), aspect.ratio=1/1, plot.margin=unit(c(1,1,1,1),"lines"))+ 
  labs(y=expression(Lesion ~ Area ~ (cm^{2})))
...
p3 <- ggplot(FigDat3, aes(x = Plant.Lab.Ord, y = mLS))+
  ...
  geom_line(size=0.5, aes(color=factor(Group), group=factor(Igeno)), show.legend=F, alpha=0.8)+
  facet_grid(.~SpLabs, scales="free_x")+
  theme(text = element_text(size=14), axis.text.x = element_text(angle=45, hjust=1), strip.background = element_blank(), strip.text.x=element_text(), aspect.ratio=1/1, axis.title.x=element_text(color=NA), plot.margin=unit(c(1,1,1,1),"lines"))+ 
  labs(y=expression(Lesion ~ Area ~ (cm^{2})))
...
p4 <- ggplot(FigDat3, aes(x = Plant.Lab.Ord, y = mLS))+
  ...
  geom_line(size=0.5, aes(color=factor(Group), group=factor(Igeno)), show.legend=F, alpha=0.8)+
  facet_grid(.~SpLabs, scales="free_x")+
  theme(text = element_text(size=14), axis.text.x = element_text(angle=45, hjust=1), strip.background = element_blank(), axis.title.y = element_text(), axis.title.x=element_text(color=NA), strip.text.x= element_text(), aspect.ratio=1/1, plot.margin=unit(c(1,1,1,1),"lines"))+ 
  labs(y=expression(Lesion ~ Area ~ (cm^{2})))

#fifth panel: plant varying subset
names(FigDat3)
p5 <- ggplot(FigDat3, aes(x = Plant.Lab.Ord, y = mLS))+
  theme_bw()+
  #order: all, Intx, tomato
  scale_color_manual(values = c("chartreuse3", "grey80"))+
  geom_line(size=0.5, aes(color=factor(PlFxFIX), group=factor(Igeno)), show.legend=F, alpha=0.8)+
  facet_grid(.~SpLabs, scales="free_x")+
  theme(text = element_text(size=14), axis.text.x = element_text(angle = 45, hjust = 1), strip.background = element_blank(), strip.text.x=element_text(), axis.title.x=element_text(color=NA), aspect.ratio=1/1, plot.margin=unit(c(1,1,1,1),"lines"))+ 
  labs(y=expression(Lesion ~ Area ~ (cm^{2})))
...
p6 <- ggplot(FigDat3, aes(x = Plant.Lab.Ord, y = mLS))+
  ...
  facet_grid(.~SpLabs, scales="free_x")+
  theme(text = element_text(size=14), axis.text.x = element_text(angle = 45, hjust = 1), strip.background = element_blank(), strip.text.x=element_text(), axis.title.y = element_text(), axis.title.x=element_text(color=NA), aspect.ratio=1/1, plot.margin=unit(c(1,1,1,1),"lines"))+ 
  labs(y=expression(Lesion ~ Area ~ (cm^{2})))
...
#alternates: "indianred1"   "orchid3"    "darkgoldenrod1"  "orange" "chartreuse3"
############################
tiff("plots/poster/DomestRight/Sl_LesionSize_Intx_a.tif", width=6, height=4, units='in', res=600)
p1 
dev.off()

tiff("plots/poster/DomestRight/Sl_LesionSize_Intx_b.tif", width=6, height=4, units='in', res=600)
p2
dev.off()

tiff("plots/poster/DomestRight/Sl_LesionSize_Intx_c.tif", width=6, height=4, units='in', res=600)
p3 
dev.off()

tiff("plots/poster/DomestRight/Sl_LesionSize_Intx_d.tif", width=6, height=4, units='in', res=600)
p4 
dev.off()

tiff("plots/poster/DomestRight/Sl_LesionSize_Intx_e.tif", width=6, height=4, units='in', res=600)
p5
dev.off()

tiff("plots/poster/DomestRight/Sl_LesionSize_Intx_f.tif", width=6, height=4, units='in', res=600)
p6
dev.off()

tiff("plots/poster/DomestRight/Sl_LesionSize_Intx_g.tif", width=6, height=4, units='in', res=600)
p7
dev.off()